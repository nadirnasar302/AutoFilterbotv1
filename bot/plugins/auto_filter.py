...
async def auto_filter(bot, update):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("Next From ⏩", callback_data=f"navigate(0|next|{query})")
                ]
            )
        
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f"🙏 Page 1/{len_result if len_result < max_pages else max_pages} 🙏 page", callback_data="ignore")
        ])
